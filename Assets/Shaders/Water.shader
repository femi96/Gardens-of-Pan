// thanks @ https://github.com/thelindseyreid/
	...
	SubShader {
		...
		Pass { // Distortion
			...
			v2f vert(appdata i) {
				...
				float4 normal4 = float4(i.normal, 0.0);
		...
		Pass {
			...
			v2f vert(appdata i) {
				...
				o.pos.x += cos(_Time*_WaveSpeed*noiseSample)*_WaveAmp + _ExtraHeight;
			...
			float4 frag(v2f i) : COLOR {
				...
				float4 s = UNITY_PROJ_COORD(i.screenPos);
